...
- [x] kotlin-winrt projection
  ...
  - 2026-06-06 已按最新 kotlin-winrt README 跟进：`skiko-winui` 作为 reusable library 不再配置 `winRt { application { } }`。Skiko library 只通过显式 `type(...)` 声明自身需要的最小 WinRT projection；不依赖 `winrt-projections-windows-sdk` / `winrt-projections-windows-app-sdk` 全投影 artifacts，全投影应由最终应用按需声明。
  - 2026-06-08 已移除 `skiko.winui.useKotlinWinRtComposite` / `kotlinWinRt.localRepo` composite build 入口，以及 `skiko.winui.localWinRtRuntimeJar` / `skiko.winui.localWinRtAuthoringJar` / `skiko.winui.localWinRtRuntimeKlib` file fallback；`kotlin-winrt` plugin、runtime、authoring 和 Native klib 只能从 Maven snapshot 坐标解析。
  - 当前 kotlin-winrt plugin/artifacts 需要 JDK 25 加载；根 Gradle wrapper 已升到 9.4.0，本地验证脚本改用当前仓库 Gradle wrapper，不再借用 sibling `kotlin-winrt` wrapper。
  ...
  - 新版 kotlin-winrt generator worker 依赖通过 Maven artifact 解析；本仓库不再保留 kotlin-winrt composite debug path。
...
- [x] Samples
  ...
  - 2026-06-09 诊断 Compose 文字发糊：Clock sample 不明显是因为以线条/简单填充为主；WinUI backend 之前只按 DPI resize swapchain buffer，没有对 `IDXGISwapChain2` 设置 inverse `SetMatrixTransform`，且 backbuffer 尺寸直接 floor，非整数 DPI 下会让 WinUI/DXGI 对 swapchain 内容做额外缩放，文字最容易糊。已改为 scaled size round-to-nearest，并在每次 render 时按 `SwapChainPanel.compositionScaleX/Y` 设置 inverse matrix transform；这是 Skiko WinUI backend 侧问题，不应要求下游 Compose 特殊调用。
  - `samples/SkiaWinUISample` 默认通过 Maven 坐标消费 `io.github.compose-fluent:skiko-winui`、`io.github.compose-fluent:skiko-winui-windows` 和 `io.github.compose-fluent:winrt-runtime-jvm`；`skiko.winui.dependencyMode=local` 也走同一组 Maven 坐标并优先从受限 `mavenLocal()` 解析本地 Skiko 发布物，不再使用 `files(...)` 直接依赖 jar。需要“远端 kotlin-winrt、本机 skiko-winui”验证时使用 `-Pskiko.winui.useLocalProject=true` 把 sample settings 中的 `:skiko-winui` 指到 `../../skiko/skiko-winui`，保留 Gradle variant/identity 传播。
  - 2026-06-06 sample dependency helper 继续只声明 sample 运行 Skiko WinUI 所需坐标：`skiko-winui`、`skiko-winui-windows` 和 `winrt-runtime-jvm`。不在 Skiko sample helper 中加入 Windows SDK / Windows App SDK 全投影 artifacts；需要全投影的最终应用应自行声明。
  - 2026-06-06 复核“不要用全投影”后，`samples/SkiaWinUISample` local/maven helper 已移除 `winrt-projections-windows-sdk` / `winrt-projections-windows-app-sdk`，sample compile 仍通过。
  - `samples/SkiaWinUISample` 已按新版 kotlin-winrt README 切到最终 app module 模型：应用 `io.github.composefluent.winrt` plugin，配置 `winRt { application { } }`；不再由用户代码手动调用 `RuntimeScope.initializeSingleThreaded()` 或 Windows App SDK bootstrap。启动入口已使用 `Application.start { SkiaWinUISampleApp() }` + `Application.onLaunched(...)`，由 kotlin-winrt app authoring metadata 注册 subclass 后创建并激活 WinUI `Window`。
  - 2026-06-07 使用远端 kotlin-winrt Maven snapshot（`winrt-gradle-plugin` `20260607.065950-24`，runtime/compiler artifacts `20260607.065647-43`）与本机 `:skiko-winui` project identity 复测：app module 不再重复声明 `nugetPackage("Microsoft.WindowsAppSDK", "2.1.3")` 时，`kotlinWinRtIdentity` 能解析到 `project :skiko-winui`，runtime assets 已从 Skiko identity 聚合出 `Microsoft.WindowsAppSDK.2.1.3` payload；direct callback host 可创建 `Skia WinUI Sample` 窗口。
  - 2026-06-07 使用远端 kotlin-winrt Maven snapshot（`winrt-gradle-plugin` `0.1.0-20260607.101153-25`，runtime/compiler artifacts `0.1.0-20260607.100854-44`）复测 `Application.start { SkiaWinUISampleApp() }` + `Application.onLaunched(...)`：app module 不再重复声明 Windows App SDK NuGet，dependency identity 从本机 `:skiko-winui` project dependency 聚合到 final app；`authored-candidates.tsv`、compiler IR authored candidates、TypeDetails registrar、host WinMD/manifest 均包含 `SkiaWinUISample.SkiaWinUISampleApp`。`stageWinRtRuntimeAssets buildWinRtApplicationHost --rerun-tasks` 构建通过，启动 `build/kotlin-winrt/application-host/bin/SkiaWinUISample.exe` 后进程存活且 `MainWindowTitle=Skia WinUI Sample`、`MainWindowHandle != 0`，确认远端 snapshot 已修复 subclass `onLaunched` 路径。
  ...
  - 2026-06-07 `skiko-winui` library 默认 `skiko.winui.jvmTarget` / toolchain 已改为 25，和当前 kotlin-winrt runtime/authoring artifacts 的 JVM baseline 对齐；仍允许通过 Gradle property 显式覆盖，但默认 project dependency 验证不再暴露 JVM 22 variant。
...
- [x] Gradle organization
  ...
  - 2026-06-06 run `27050847138` 通过：`Verify skiko-winui publish locally` 和 `Publish skiko-winui` 均成功，确认最新 kotlin-winrt snapshot 用法对应的 `skiko-winui` snapshot 已推送成功。
  - 2026-06-06 run `27064450341` 通过：撤回 `winrt-projections-*` 全投影传递依赖后，`Verify skiko-winui publish locally` 和 `Publish skiko-winui` 仍成功，确认 snapshot 已更新为 Skiko 最小 projection 生成模型。
...
- [x] Replace local sibling validation flags with Maven coordinates.
  ...
  - 2026-06-08 已删除默认 settings 中的 sibling `../kotlin-winrt` composite 分支；`skiko-winui` buildscript 和 `samples/SkiaWinUISample` buildscript 也不再通过 `mavenLocal()` 解析 kotlin-winrt plugin。
  - 2026-06-08 已删除本地 winrt runtime/authoring jar/klib file fallback；`.agent_scripts/run_windows_gradle.ps1` 不再要求 sibling `kotlin-winrt` checkout。
  - 2026-06-08 根 Gradle wrapper 从 8.13 升到 9.4.0；否则 JDK 25 下本仓库 wrapper 配置阶段报 `25.0.3`，会重新迫使本地验证借 sibling kotlin-winrt wrapper。
...
- [x] `:skiko-winui:generateWinRtProjections`
  ...
  - 2026-06-08 待复跑：已移除上面命令中的 local winrt jar 参数；后续验证应只保留 `localSkikoJar`，kotlin-winrt 一律走 Maven snapshot。
...
- [x] `samples/SkiaWinUISample:compileKotlin`
  ...
  - 2026-06-07 按“远端 kotlin-winrt、本机 skiko-winui”验证：先用 `--refresh-dependencies` 拉到远端 snapshot（`winrt-gradle-plugin` `0.1.0-20260607.065950-24`，runtime/compiler artifacts `0.1.0-20260607.065647-43`），期间 Sonatype snapshot metadata 曾因 TLS handshake 抖动失败；不带 refresh 复跑 `..\..\..\kotlin-winrt\gradlew.bat --no-daemon --console=plain -p . "-Dkotlin.daemon.jvmargs=-Xmx8192m" "-Dorg.gradle.jvmargs=-Xmx8192m" "-Pskiko.winui.useLocalProject=true" "-Pskiko.winui.dependencyMode=maven" "-Pskiko.winui.version=0.0.0-SNAPSHOT" "-Pskiko.winui.skipSkikoComposite=true" "-Pskiko.winui.jvmTarget=25" "-Pskiko.winui.jvmToolchain=25" "-Pskiko.winui.mingw.enabled=false" "-Pskiko.winui.localSkikoJar=E:\Documents\AndroidStudioProjects\compose-fluent-skiko\skiko\build\libs\skiko-awt-0.0.0-SNAPSHOT.jar" "-Pskiko.winui.vsPath=D:\Program Files\Microsoft Visual Studio\2022\Community" stageWinRtRuntimeAssets buildWinRtApplicationHost --rerun-tasks` 通过。
  - 2026-06-07 `onLaunched` 远端修复复测结果：`--refresh-dependencies` 拉到 `winrt-gradle-plugin` `0.1.0-20260607.101153-25`、runtime/compiler artifacts `0.1.0-20260607.100854-44` 后，`Application.start { SkiaWinUISampleApp() }` + `override fun onLaunched(...)` 构建通过；`authored-candidates.tsv` 为 185 bytes，包含 `SkiaWinUISample.SkiaWinUISampleApp` / `Microsoft.UI.Xaml.IApplicationOverrides`，`authored-metadata.tsv` 为 155 bytes，registrar 调用 `WinRT_SkiaWinUISampleApp_TypeDetails.register()`。
  - 2026-06-07 启动 `SkiaWinUISample.exe` 验证 subclass path：8 秒后 PID `19300`、`MainWindowHandle=4654118`、`MainWindowTitle=Skia WinUI Sample`、`Responding=True`；随后关闭进程。结论：远端 kotlin-winrt snapshot 已修复 app subclass `onLaunched` authoring，sample 保持 `Application.start { SkiaWinUISampleApp() }`。
  - 2026-06-08 移除 external kotlin-winrt 后复测 sample host：使用当前仓库 Gradle 9.4 wrapper 执行 `.\gradlew.bat --no-daemon --console=plain -p samples\SkiaWinUISample "-Dkotlin.daemon.jvmargs=-Xmx8192m" "-Dorg.gradle.jvmargs=-Xmx8192m" "-Pskiko.winui.useLocalProject=true" "-Pskiko.winui.dependencyMode=maven" "-Pskiko.winui.version=0.0.0-SNAPSHOT" "-Pskiko.winui.skipSkikoComposite=true" "-Pskiko.winui.jvmTarget=25" "-Pskiko.winui.jvmToolchain=25" "-Pskiko.winui.mingw.enabled=false" "-Pskiko.winui.localSkikoJar=E:\Documents\AndroidStudioProjects\compose-fluent-skiko\skiko\build\libs\skiko-awt-0.0.0-SNAPSHOT.jar" "-Pskiko.winui.vsPath=D:\Program Files\Microsoft Visual Studio\2022\Community" stageWinRtRuntimeAssets buildWinRtApplicationHost`，结果通过；sample buildscript 不再通过 `mavenLocal()` 解析 kotlin-winrt plugin。
  - 2026-06-09 swapchain DPI matrix 修复后复测：`.\gradlew.bat --no-daemon --console=plain -p . "-Dkotlin.daemon.jvmargs=-Xmx8192m" "-Dorg.gradle.jvmargs=-Xmx8192m" "-Pskiko.winui.skipSamples=true" "-Pskiko.winui.skipSkikoComposite=true" "-Pskiko.winui.jvmTarget=25" "-Pskiko.winui.jvmToolchain=25" "-Pskiko.winui.mingw.enabled=false" "-Pskiko.winui.localSkikoJar=skiko/build/libs/skiko-awt-0.0.0-SNAPSHOT.jar" "-Pskiko.winui.vsPath=D:\Program Files\Microsoft Visual Studio\2022\Community" :skiko-winui:compileWinuiJvmNativeWindowsX64 :skiko-winui:compileKotlinWinuiJvm` 通过；随后 `samples\SkiaWinUISample` 的 `stageWinRtRuntimeAssets buildWinRtApplicationHost` 通过，直接启动 exe 后 PID `36940`、`MainWindowHandle=264831244`、`MainWindowTitle=Skia WinUI Sample`、`Responding=True`，随后关闭进程。
  - 2026-06-10 合并 `origin/master`（上游 buildSrc / test-utils 重构）后复测：`:skiko-winui:compileWinuiJvmNativeWindowsX64 :skiko-winui:compileKotlinWinuiJvm` 通过；`samples\SkiaWinUISample` host 本机复跑两次失败在 `skiko:buildSrc:compileKotlin` 解析 `cache-redirector.jetbrains.com` / Google Maven 时 TLS handshake 被远端断开，`--offline` 复跑又因本机缺少上游重构后的 buildSrc 依赖缓存失败。该失败发生在 sample included build 的外部依赖解析阶段，不是 Skiko WinUI 源码或 kotlin-winrt runtime failure；推送后由 GitHub Actions 干净环境继续验证 sample/publish。
  - 2026-06-10 CI `Publish Skiko WinUI` run `27226451201` 失败在 `:skiko-winui:compileWinuiSkikoWindowsX64` 的 clean Skia 目录校验：上游 Windows Skia unzip 会把 `libpng.lib` / `libjpeg.lib` / `libwebp*.lib` 规范化为 `png.lib` / `jpeg.lib` / `webp*.lib`，而 WinUI native link 列表仍使用旧 `lib*` 名称。本次已改为与上游 `skiko/build.gradle.kts` 的 `staticSkiaLibs("png", "jpeg", "webp", "webp_sse41")` 保持一致。
  - 2026-06-10 本机为验证新库名，在生成的 Skia 依赖目录补出 `png.lib` / `jpeg.lib` / `webp.lib` / `webp_sse41.lib` 后复跑 `:skiko-winui:publishSkikoWinuiToMavenLocal`；缺库校验通过，但本机 native compile 后续失败为 `compile-skiko-winui-windows-launcher.log` 报 `The process cannot access the file because it is being used by another process.`，不是 clean CI 中的缺库问题。修复推送后继续由 GitHub Actions 验证完整 publish。
  - 2026-06-10 修复 Skia Windows 库名后 CI `Publish Skiko WinUI` run `27228282786` 通过；`Download Skia Windows dependency`、`Verify skiko-winui publish locally`、`Publish skiko-winui` 全部成功。
  - 2026-06-04 曾发现 `Application.start { SkiaWinUISampleApp() }` + `onLaunched(...)` 在 sample app 中没有创建窗口；诊断结果是 sample 走 application-packaging-only 时 `authored-candidates.tsv` / `authored-metadata.tsv` 为空，Application subclass override 没有进入 authoring TypeDetails，WinUI 不会回调 `onLaunched`。2026-06-07 远端 kotlin-winrt snapshot 已修复该问题。
...
- [x] `:skiko-winui:publishSkikoWinuiToMavenLocal`
  ...
  - 2026-06-06 最新 kotlin-winrt snapshot/README 跟进后复跑通过。命令：`$env:JAVA_HOME='C:\Program Files\Microsoft\jdk-25.0.3.9-hotspot'; $env:PATH="$env:JAVA_HOME\bin;$env:PATH"; ..\kotlin-winrt\gradlew.bat --no-daemon --console=plain -p . "-Pskiko.winui.skipSkikoComposite=true" "-Pskiko.winui.jvmTarget=25" "-Pskiko.winui.jvmToolchain=25" "-Pskiko.winui.mingw.enabled=false" "-Pskiko.winui.localSkikoJar=skiko/build/libs/skiko-awt-0.0.0-SNAPSHOT.jar" "-Pskiko.winui.vsPath=D:\Program Files\Microsoft Visual Studio\2022\Community" :skiko-winui:publishSkikoWinuiToMavenLocal --rerun-tasks`。结果：通过；`generateWinRtProjections`、`compileKotlinWinuiJvm`、`generatePomFileForSkikoWinuiJvmPublication` 均运行。随后已按要求撤回全投影传递依赖，主 POM 只声明 `winrt-runtime-jvm` 和 `skiko-winui-windows`。
  - 2026-06-06 全投影依赖撤回后验证主 JVM publication 通过。命令：`$env:JAVA_HOME='C:\Program Files\Microsoft\jdk-25.0.3.9-hotspot'; $env:PATH="$env:JAVA_HOME\bin;$env:PATH"; ..\kotlin-winrt\gradlew.bat --no-daemon --console=plain -p . "-Pskiko.winui.skipSkikoComposite=true" "-Pskiko.winui.jvmTarget=25" "-Pskiko.winui.jvmToolchain=25" "-Pskiko.winui.mingw.enabled=false" "-Pskiko.winui.localSkikoJar=skiko/build/libs/skiko-awt-0.0.0-SNAPSHOT.jar" "-Pskiko.winui.vsPath=D:\Program Files\Microsoft Visual Studio\2022\Community" :skiko-winui:publishSkikoWinuiJvmPublicationToMavenLocal --rerun-tasks`。结果：通过；POM 检查确认没有 `winrt-projections-*` 依赖。
...
- [x] `:skiko-winui:tasks --all` publish task configuration.
  - 最近结果：2026-06-08 通过。命令：`$env:JAVA_HOME='C:\Program Files\Microsoft\jdk-25.0.3.9-hotspot'; $env:PATH="$env:JAVA_HOME\bin;$env:PATH"; .\gradlew.bat --no-daemon --console=plain -p . "-Dkotlin.daemon.jvmargs=-Xmx8192m" "-Dorg.gradle.jvmargs=-Xmx8192m" "-Pskiko.winui.skipSamples=true" "-Pskiko.winui.skipSkikoComposite=true" "-Pskiko.winui.jvmTarget=25" "-Pskiko.winui.jvmToolchain=25" "-Pskiko.winui.mingw.enabled=false" "-Pskiko.winui.localSkikoJar=skiko/build/libs/skiko-awt-0.0.0-SNAPSHOT.jar" "-Pskiko.winui.vsPath=D:\Program Files\Microsoft Visual Studio\2022\Community" :skiko-winui:tasks --all`。
  - 2026-06-08 已移除 sibling `kotlin-winrt` wrapper/composite 路径；后续本地验证应使用当前仓库 `gradlew.bat` 或 CI 安装的 Gradle，不能借用 `..\kotlin-winrt\gradlew.bat`。
