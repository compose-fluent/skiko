...
- [x] `samples/SkiaWinUISample:compileKotlin`
  ...
  - 2026-06-07 `onLaunched` 复测结果：临时改为 `Application.start { SkiaWinUISampleApp() }` + `override fun onLaunched(...)` 后构建通过，但 `build\classes\kotlin\main\kotlin-winrt\authored-candidates.tsv` 为空、`build\classes\kotlin\main\kotlin-winrt-authoring\authored-metadata.tsv` 为空，生成 registrar 的 `register()` 仍为空；启动 `SkiaWinUISample.exe` 8 秒后 PID `42088`、`MainWindowHandle=0`、`MainWindowTitle=`、`Responding=True`。结论：最新远端 kotlin-winrt snapshot 仍未修复 app subclass `onLaunched` authoring，sample 已恢复 direct callback。
  - 2026-06-07 恢复 direct callback 后复测 host：同一命令不带 `--rerun-tasks` 构建通过；直接启动 `SkiaWinUISample.exe`，6 秒后 PID `38748`、`MainWindowHandle=51383336`、`MainWindowTitle=Skia WinUI Sample`、`Responding=True`，随后关闭进程。
